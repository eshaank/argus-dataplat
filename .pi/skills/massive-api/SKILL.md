---
name: massive-api
description: Access Massive(Polygon) stock, crypto, forex, options, indices, futures, market data, and news APIs via CLI. NOTE — In the DataPlat architecture, Polygon/Massive is used ONLY for reference metadata (universe, sectors, SIC codes), news, and corporate actions. It is NEVER used for historical prices, OHLCV, quotes, or any per-ticker market data. Schwab is the sole source for all ticker-level data. See the dataplat skill for details.
metadata:
  openclaw:
    requires:
      bins: ["npx"]
